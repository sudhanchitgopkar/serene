...
function populateTables(safest, mostVolatile) {
    ...
    if (safest != null && safest != undefined) {
        for (var i = 0; i < Object.keys(safest).length; i++) {
            document.getElementById('hv' + i).innerHTML = safest[i]['ticker'] + ': ' + parseInt(parseFloat(safest[i]['volatility']) * 100);
        } // for i
    } // if
    ...
    if (mostVolatile == null || mostVolatile == undefined) {
        ...
        document.getElementById('hvadvice').innerHTML = 'One or more holdings have a voli which exceed your volatility threshold';
        if (safest == null || safest == undefined) {
